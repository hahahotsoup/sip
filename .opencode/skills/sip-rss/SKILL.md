...
| `sip --grep <关键词> [--brief] [--json]` | 全文搜索（标题/正文/摘要关键字匹配，不依赖 AI；`--brief` 截断摘要防刷屏，`--json` 结构化输出） |
...
1. **先用全文搜索确认命中**：`--grep` 是精确关键字匹配（标题/正文/摘要），不依赖 AI、无阈值问题，最适合先跑。**命中多或来源正文很长时加 `--brief`**（摘要截断到 200 字符）避免刷屏；需要结构化结果用 `--json`。
