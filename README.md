...
按「五动词」归类，帮你快速知道每一环能做什么：

| 动词 | 承载功能 |
|------|---------|
| 📥 **收集** | RSS / RSSHub 订阅源、OPML 导入 |
| 💾 **保存** | 全文抓取、版本快照、阅读进度 |
| 🔎 **追踪** | 版本历史、内容 Diff、「被作者改过」 |
| 🧹 **筛选** | Insights 报告、Source Policy 规则、跨源去重、高频源折叠、今日变化摘要 |
| 🚀 **使用** | 全文/语义搜索、Agent / Bot 接入、Markdown 导出 |

> 加功能前先问一句：它属于哪一环？——既是产品边界，也防止概念被撑爆。每项细节见 [Wiki · 功能](https://sip.hotsouprealm.top/参考/功能.html)。
...
sip 只做两件事——**确定性的规则** 与 **本地事实的存储**。你在意的收集、保存、追踪、筛选、使用，全都建立在这两件事之上；凡是需要判断、解释、对话的，都交给 agent 与用户。它不会让你刷更多，但会让你读得更安心。
...
> 这些步骤的目标是把 sip 从「RSS 阅读器」变成「个人信息库」——Insights 是分析层，Source Policy 让「你的决定」持久生效，onboarding 降低首次使用门槛。
