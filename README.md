...
1. **术语与事实分离**——报告区分「状态」（仅技术故障：拉取失败/长期未更新）与「活跃度」（关于你的行为），**低阅读 ≠ 低价值**；删除一切把行为包装成价值判断的措辞。
2. **不引入黑盒评分**——不产出 `source_score = 0.72` 这类无法解释的数字；任何「建议」都必须带**可解释的原因链**（指标 + 数值 + 人类可读说明），让用户自己判断。
3. **Source Policy 闭环**——从「用户发现低价值源→手动处理」升级为「Insights → 生成建议 → **用户确认** → source rule → 信息流调整」：

   ```
   读 → 分析 → 调整输入 → 更好的信息流
   ```

   规则（归档 / 降频 / 打标签 / 退订候选）**全部由用户确认**，`createdBy` 永远是 `user`，AI 永不自动写入。

> 这些步骤的目标是把 sip 从「信息仓库」变成「信息摄入管理系统」——Insights 只是起点，Source Policy 与个性化过滤串起来后，才构成完整闭环。
