...
| `M`（或命令行 `manage`） | 打开「订阅源管理页」：全屏列出所有源（计划/上次/文章数），`j/k` 移动、`u` 更新、`a` 归档、`r` 去归档、`x` 删除、`s` 设计划、`d` 加源 |
...
> **阅读进度记忆**：每篇文章的滚动位置会记住（存在 `readwithhotsoup/reading_progress.json`，不改数据库）——退出 TUI 或切换文章后再回来，会从上次读到的地方继续，而不是回到开头。
...
**全屏阅读**：`sip --show <文章编号>` 打开一个无侧栏的全屏阅读界面（Markdown 渲染正文），底部提示 **「按 W 进入完整阅读器 · 按 Esc 退出」**——按 `W` 无缝切入完整 TUI（并定位到当前文章），按 `Esc`/`Q` 返回命令行。文章编号用 `sip --grep 关键词`、`sip --search` 或 `sip -l <源编号>` 结果里的编号。

**AI 读原文**：`sip --show <文章编号> --json` 把文章的标题/来源/链接/发布时间/作者 + **原始正文**（Content 原文，不做任何渲染）以 JSON 打到标准输出，供 AI 或脚本读取，例如 `sip --show 42 --json --lang en-US --ignoresafeannouncement`。

| 短参数 | 长参数 | 说明 |
|--------|--------|------|
| `-l` | `--list` | 列出所有订阅源；带编号则列出该源的文章（如 `-l 1`）。`-l 1` 的编号格式为 `[列表序号/文章真实ID]`，`--show/--versions/--summary` 等命令用右边的真实 ID |
...
> **CLI 模式下不会自动同步**，但会在命令执行完后提醒你：若有到期订阅源，输出 `有 N 个订阅源已到期，运行 sip --sync 可更新`（仅提示、不自动拉取，零网络开销）。脚本 / AI Agent 可用 `--ignoresafeannouncement` 关闭该提示；`--json` 输出不受影响。
...
所有用户可见文案都从 `readwithhotsoup/languages/<代码>.json` 读取，**源码原文是英文**，语言文件为「英文键 → 译文」，缺失时回退英文原文。
