...
- **今日哈汤**：`sip --today` 给出今日 5 篇规则式选文（近 48h 新增 / 近期被作者更新 / 全文质量 / ♥🤖 标记加权，附预估阅读时长与理由），起始页同步展示——**先引导每日少量阅读的习惯**，个性化排序等 telemetry 数据积累后再演进
...
- 📖 **Sip Today 个性化**：v1 是规则式选文；等 telemetry 积累足够行为数据后，演进出「为什么推荐这篇」的个性化排序
...
| `--like <编号> [--ai [理由]]` | | 标记文章：用户点赞（♥）或 AI 判断（🤖）；`--likes` 查看 |
| `--today [--json]` | | 今日阅读清单（规则式选文 5 篇 + 目标/完成进度） |
| `--sync` | | 只更新「到期」的订阅源（可选 `--feed 编号` 限定单个；`--json` 结构化输出） |
